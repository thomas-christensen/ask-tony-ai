      ...
      <div className="w-full h-[280px]">
            <ChartContainer config={chartConfig} className="h-full w-full aspect-auto">
              <RechartsLineChart data={chartData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
                {showGrid && <CartesianGrid strokeDasharray="3 3" stroke="hsl(0, 0%, 80%)" strokeOpacity={0.5} />}
                <XAxis 
                  dataKey="name"
                  tickLine={false}
                  axisLine={false}
                ...
                {Object.keys(chartConfig).map((key) => {
                  const lineConfig = chartConfig[key];
                  const strokeColor = lineConfig?.color || CHART_COLORS[0];
